...
export default class AllDocuments extends React.Component {
                ...
                ${this.props.customColumns
                    ...
                    for (const file of rows) {
                        ...
                        if (officeExtensions.includes(extension || "")) {
                            ...
                            editUrl = editUrl = `${window.location.origin}${libPath}?id=${encodeURIComponent(serverRelativePath)}&parent=${encodeURIComponent(parentPath)}`;
                        }
                        else {
    ...
    applyFilters(items) {
        ...
        if (sortConfig) {
            ...
            filtered.sort((a, b) => {
                const valA = column === "name" ||
                    column === "modified" ||
                    column === "modifiedBy" ||
                    column === "library"
                    ? a[column]
                    : a.customColumns[column] || "";
                const valB = column === "name" ||
                    column === "modified" ||
                    column === "modifiedBy" ||
                    column === "library"
                    ? b[column]
    ...
    render() {
        ...
        return (React.createElement("div", { className: styles.container },
            React.createElement("div", { style: { marginBottom: "1rem" } },
                React.createElement("input", { type: "text", placeholder: "...", value: this.state.searchTerm, onChange: this.onSearchChange, className: styles.searchBox })),
            this.props.customColumns.map((col) => (React.createElement("div", { key: col.internalName, className: styles.filterWrapper },
                ...
                React.createElement("select", { className: styles.filterDropdown, onFocus: (e) => {
                        e.currentTarget.style.borderColor = "#0078d4";
                    }, onBlur: (e) => {
                        e.currentTarget.style.borderColor = "#8a8886";
                    }, id: col.internalName, value: filters[col.internalName] || "", onChange: (e) => this.onFilterChanged(col.internalName, e) },
            ...
            React.createElement("table", { className: styles.fluentliketable },
                ...
                React.createElement("tbody", null, filteredItems.map((item, idx) => {
                    var _a;
                    return (React.createElement("tr", { key: idx },
                        React.createElement("td", null,
                            React.createElement(Icon, Object.assign({}, getFileTypeIconProps({
                                extension: (_a = item.name.split(".").pop()) === null || _a === void 0 ? void 0 : _a.toLowerCase(),
                                size: 32,
                                imageFileType: "svg",
